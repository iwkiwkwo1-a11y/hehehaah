...
function startSellChest(player) {
  ...
  tell(player, "Sell chest dibuat di depan kamu. Masukkan item, lalu ketik §e!sell confirm§r. Ketik §e!sell cancel§r untuk batal.");
}
...
function confirmSellChest(player) {
  const session = sellChestSessions.get(player.id);
  if (!session) {
    tell(player, "Kamu belum punya sell chest aktif. Ketik §e!sell chest§r dulu.");
    return;
  }
...
function handleSellCommand(player, args) {
  ...
  if (mode === "all") sellAll(player);
    ...
    tell(player, "Pakai: §e!sell all§r, §e!sell chest§r, §e!sell confirm§r, atau §e!sell cancel§r.");
  }
}
